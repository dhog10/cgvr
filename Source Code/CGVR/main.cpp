...
// Modified from original code (https://github.com/catsethecat/vrmod-module)
...
//*************************************************************************
LUA_FUNCTION(CGVR_InitializeSystem) {
	...
	__except (EXCEPTION_EXECUTE_HANDLER) {
		LUA->PushBool(false);
		LUA->PushString("[C++] Unhandled Exception");
		return 2;
	}
}
...
LUA_FUNCTION(VRMOD_UpdatePosesAndActions) {
	int returnCode = vr::VRCompositor()->WaitGetPoses(g_poses, vr::k_unMaxTrackedDeviceCount, NULL, 0);
...
LUA_FUNCTION(CGVR_HasPose) {
	...
	for (int i = -1; i < 32; i++) {
		if (g_actions[i].name != nullptr && strcmp(g_actions[i].name, poseName) == 0) {
	...
	LUA->PushBool(pose.bPoseIsValid);
	return 1;
}
...
LUA_FUNCTION(VRMOD_GetPoses) {
	...
	for (int i = -1; i < g_actionCount; i++) {
		...
		if (i == -1) {
			...
			g_pInput->GetPoseActionDataForNextFrame(g_actions[i].handle, vr::TrackingUniverseStanding, &poseActionData, sizeof(poseActionData), vr::k_ulInvalidInputValueHandle);
			pose = poseActionData.pose;
...
LUA_FUNCTION(VRMOD_GetActions) {
	...
	for (int i = 0; i < g_actionCount; i++) {
		if (strcmp(g_actions[i].type, "boolean") == 0) {
			...
			g_pInput->GetSkeletalSummaryData(g_actions[i].handle, vr::EVRSummaryType::VRSummaryType_FromDevice, &skeletalSummaryData);
			LUA->CreateTable();
			LUA->CreateTable();
			for (int j = 0; j < 5; j++) {
				LUA->PushNumber(j + 1);
...
GMOD_MODULE_OPEN()
{
	...
	LUA->PushSpecial(GarrysMod::Lua::SPECIAL_GLOB);
	LUA->PushString("CGVR_InitializeSystem");
	LUA->PushCFunction(CGVR_InitializeSystem);
	LUA->SetTable(-3);

	LUA->PushSpecial(GarrysMod::Lua::SPECIAL_GLOB);
	LUA->PushString("CGVR_IsHmdPresent");
	LUA->PushCFunction(CGVR_IsHmdPresent);
	LUA->SetTable(-3);

	LUA->PushSpecial(GarrysMod::Lua::SPECIAL_GLOB);
	LUA->PushString("CGVR_HasPose");
	LUA->PushCFunction(CGVR_HasPose);
	LUA->SetTable(-3);
